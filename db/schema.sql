...
-- ----------------------------------------------------------------------------
-- f_findings + notification_channels — alerting pipeline (009).
-- The ingester's threshold evaluator (ingestion/findings.py) emits one
-- structured finding per detected condition; delivery channels render that
-- one shape (SNS today, more later). Findings have a lifecycle (active/
-- resolved, dedup by finding_id); notifications fire on state TRANSITIONS.
-- recommended_action carries the prepared CLI/console action — the dashboard
-- itself never writes to monitored accounts.
-- ----------------------------------------------------------------------------
