...
export const SECTION_INFO = {
  ...
  'ops-exec-summary': {
    ...
    body: 'A narrative analysis of your fleet\'s last-N-days Bedrock posture, written by the Ops Review AI agent (Claude on Amazon Bedrock) from the structured findings on this page. The agent is grounded only in the metrics shown — it cannot see raw data and never invents numbers. Includes a traffic-flow Mermaid diagram when there is enough data.',
  ...
  'notifications': {
    title: 'Notifications & findings',
    body: 'After every ingest run the dashboard evaluates the freshly-loaded data against your thresholds and maintains a set of FINDINGS: quota utilization vs applied limits (burndown-weighted, same math as the Quotas tab), throttle rate per account/model/region, day-over-average cost jumps, and models still serving traffic within N days of Legacy/EOL. A finding opens when a threshold is crossed and resolves when the condition clears - notifications fire on those transitions only, so a persistent condition alerts once, not on every run. Delivery is via an SNS topic in this account: email subscribers get a readable digest; Lambda/webhook/EventBridge subscribers get the structured findings JSON, so Slack (AWS Chatbot), PagerDuty, or ticketing integrate without code changes here.',
    why: 'Single-metric CloudWatch alarms can\'t express these signals - "peak TPM as % of the applied quota" is a join across CloudWatch and Service Quotas, and cost/lifecycle findings aren\'t CloudWatch metrics at all. Centralizing the evaluation here means one set of thresholds covers every monitored account instead of hundreds of per-account alarms. For minute-level thresholds on a single metric in one account, a native CloudWatch alarm is still the right tool - the two are complements.',
    action: 'Set the SNS topic ARN and use "Send test notification" to verify delivery end-to-end. Every finding carries a PREPARED action - a ready-to-run CLI command and console deep link (e.g. a pre-filled Service Quotas increase request) - which you review and execute with your own credentials; the dashboard never modifies monitored accounts. Findings are also always visible in the bell menu in the top navigation.',
    docLink: 'https://docs.aws.amazon.com/sns/latest/dg/welcome.html',
  },
  'workloads-setup': {
  ...
  'wl-table': {
    ...
    action: 'Export for chargeback or a review meeting. A workload with high p99 latency but low volume → likely large prompts or a slow model choice; check its token percentiles. High error % → engage that use-case\'s owner.\n\nRelated: when this tab is pivoted by "user", it overlaps with the By User tab — but the two answer different needs. By User is AWS-witnessed (from invocation-log identity — audit-friendly, runtime-only, volume+tokens). This tab is client-reported (proxy/OTEL — needs emitter opt-in, but adds throttle, latency, TTFT, estimated cost, and covers Mantle plus direct Anthropic/OpenAI API traffic). The README "Which attribution source when?" table compares all sources.',
  },
  'wl-by-provider': {
    title: 'Usage by provider (client-reported)',
    body: 'Requests, tokens, errors, retries, latency/TTFT, and estimated cost rolled up by model PROVIDER (Anthropic, OpenAI, Amazon, …) and by PATH (bedrock-runtime, bedrock-mantle, or the direct anthropic-api / openai-api endpoints). The direct-API rows exist only in client telemetry — no AWS-side source (CloudWatch, invocation logs, Cost Explorer) can see traffic that never touches Bedrock. All rows here are client-reported: only instrumented or proxied traffic appears.',
    why: 'Most organizations run more than one provider and more than one path to the same provider. This is the single view that answers "what is our total Anthropic usage — Bedrock AND direct API — vs our OpenAI usage", including client-experienced latency, TTFT, and retry counts that server-side metrics never carry. For bedrock-mantle it is also the only latency source besides invocation logs.',
    action: 'Est. cost is the emitters\' per-request estimate — reconcile against Cost Explorer before using it for billing. Rows for a path you did not expect (e.g. direct openai-api traffic) are a governance signal: that traffic bypasses your Bedrock controls. To populate: emit client telemetry via your proxy (LiteLLM callback), an OTEL collector, or Claude Code\'s built-in telemetry — see the client-telemetry section of the README.',
  },
  'identity-usage': {
  ...
  'lifecycle-timeline': {
    ...
    body: 'Horizontal timeline of the top 8 legacy/EOL models that are actively in use in the selected window. Each row is one model; the colored band runs from its Legacy date to its EOL date. The vertical line on every row is today. The Legacy / Extended access / EOL milestones are dotted on each band. Current-policy models (launched on or after 7 Sep 2026) show no extended-access dot — that phase does not exist for them.',
    ...
    action: 'CRITICAL bands (red) crossing through today → migrate now, you are on borrowed time. WARNING bands (orange) → plan migration before EOL. INFO (blue) → schedule a review with the application owner so the date is on their radar. Short bands are the ones to watch: a current-policy model can be given as little as 45 days between Legacy and EOL.',
    docLink: 'https://docs.aws.amazon.com/bedrock/latest/userguide/model-lifecycle.html',
  },
  'lifecycle-table': {
    title: 'Legacy & EOL models',
    body: 'Every Bedrock model in your portfolio that is LEGACY or already past EOL, from the live AWS API (bedrock:ListFoundationModels). Lifecycle dates and status come straight from AWS — no scraping, no bundled JSON. ACTIVE models are deliberately excluded: they need no migration work. Click a row to expand the per-account drill-down: which accounts are using each model, in which regions, and when they last accessed it.\n\nTwo policies, shown in the Policy column. Models launched on Bedrock BEFORE 7 Sep 2026 follow the legacy policy: at least 12 months on Bedrock, at least 6 months in Legacy, and (for EOL dates after 1 Feb 2026) a public-extended-access phase where the provider may raise prices. Models launched ON OR AFTER 7 Sep 2026 follow the current policy: no 12-month floor, NO extended-access phase at all, and a Legacy period of either 6 months or 45 days. "Notice given" is the model\'s actual Legacy-to-EOL gap and is flagged when it is short. Extended access reads "n/a" — not "—" — for current-policy models, because not-applicable and not-yet-published are different facts.\n\n"Removed from API" under an EOL date means AWS has deleted the model from Bedrock in every region tracked. This dashboard keeps the row anyway; requests to it now fail, so any traffic shown on that row is errors rather than usage.',
    why: 'Once a model passes its EOL date, AWS reserves the right to stop accepting requests at any time. A workload relying on an EOL model is one announcement away from a P0. Legacy models also lag in throughput, latency, and pricing — migration usually improves the bill at the same time. Knowing which accounts are affected is the difference between "send a generic notice" and "ping the three teams that matter". The policy matters for planning: under the current policy 45 days may be all the warning you get, and there is no extended-access period to fall back on.',
    action: 'CRITICAL (past EOL, EOL within 30 days, or past extended access on a legacy-policy model) → migrate this week. Use Recommended upgrade as the starting point. WARNING (currently Legacy) → plan for next sprint. INFO (Legacy within 90 days) → flag proactively. A row marked "removed from API" with non-zero requests is an active outage, not a plan item. Use Download CSV to share with stakeholders or ingest into Jira.',
    docLink: 'https://docs.aws.amazon.com/bedrock/latest/userguide/model-lifecycle.html',
  },
  ...
  'by-user-chart': {
    ...
    action: 'Group by App/Group for team-level chargeback; switch to User to find individual heavy users. Note the trust caveat: the session name is chosen by the caller — it is audit-grade only when enforced (IAM condition on sts:RoleSessionName or Identity Center federation). The role axis cannot be faked. Empty → enable Bedrock Model Invocation Logging.\n\nRelated: the "Usage · Custom Attributes" tab can ALSO break usage down by user/team — but from client-reported telemetry (proxy or OTEL) instead of AWS logs. Rule of thumb: use THIS tab when you need AWS-witnessed, audit-friendly numbers (who really called Bedrock); use Custom Attributes when you need what only clients can report — throttle/latency/TTFT per person, Mantle traffic, or direct Anthropic/OpenAI API calls. The README "Which attribution source when?" table compares them.',
    docLink: 'https://docs.aws.amazon.com/bedrock/latest/userguide/model-invocation-logging.html',
  },
  'by-user-table': {
    title: 'Callers',
